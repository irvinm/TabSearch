...
- **Version Synchronization**: The `version` field in `package.json` and `src/manifest.json` must always match and follow Semantic Versioning (`MAJOR.MINOR.PATCH`). An optional 4th component (`MAJOR.MINOR.PATCH.BUILD`) is permitted for pre-release builds to satisfy Mozilla AMO signing requirements during testing. Because TabSearch is a Firefox WebExtension marked `"private": true` and is never published to the npm registry, exact 1:1 string synchronization between `package.json` and `manifest.json` is intentional.
